...
function wrapTweets( $last_tweets, $isNew = false ) {
    $html = '';
    foreach( $last_tweets as $tweet ) {
		...
		if( isset( $last_tweets->errors ) ) {           
			...
			$html .= makeClickableLinks( $tweet->text );
        }
        $html .= '</div>';

        $date = $tweet->created_at;
        $html .= '<p data-date="' . strtotime( $date ) . '" class="tweetdate">' . twitter_time( $date ) . '</p>';
...
function twitter_time( $a, $update = false ) {
...
function logError( $message ) {
    file_put_contents( 'log_error.txt', date('H:i:s  -  ') . $message, FILE_APPEND);
>>>>>>> 0d742b84ba88c9eea1c84181da7dda098bd2e556
}